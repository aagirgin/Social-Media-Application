...
class _ProfileOthersState extends State<ProfileOthers> {
  ...
  Future<void> alreadySent() async {
    ...
    await FirebaseFirestore.instance.collection('users').doc(followingUsersAccount).get().then(
            (DocumentSnapshot documentSnapshot) {
              currNotifsRaw = documentSnapshot['userNotifications'];
        }
    );
    for (var i in currNotifsRaw) {
      currNotifs.add(Notifi.fromMap(i).userID);
    }
  ...
  Future<void> followNewUser() async {
    ...
    if (user.isPrivate) {
      ...
      Notifi newRequest = Notifi(userID: firebaseUser.uid, message: 'wants to follow you.');
      await userCollection.doc(followingUsersAccount).update({'userNotifications': FieldValue.arrayUnion([toMap(newRequest)])});
    } else {
      ...
      userCollection.doc(firebaseUser.uid).update({'userFollowing': FieldValue.arrayUnion([followingUsersAccount])});
      userCollection.doc(firebaseUser.uid).update({'followingCount': FieldValue.increment(1)});
      userCollection.doc(followingUsersAccount).update({'userFollowers': FieldValue.arrayUnion([firebaseUser.uid])});
      userCollection.doc(followingUsersAccount).update({'followersCount': FieldValue.increment(1)});
    }
    setState(() {});
  ...
  Future<void> leaveFollowing() async {
    ...
    await FirebaseFirestore.instance.collection('users').where('username', isEqualTo: user.username).get()
        .then((QuerySnapshot querySnapshot) {
      querySnapshot.docs.forEach((doc) {
        followingUsersAccount = doc.id;
      });
    });
    userCollection.doc(firebaseUser.uid).update({'userFollowing': FieldValue.arrayRemove([followingUsersAccount])});
    userCollection.doc(firebaseUser.uid).update({'followingCount': FieldValue.increment(-1)});
    userCollection.doc(followingUsersAccount).update({'userFollowers': FieldValue.arrayRemove([firebaseUser.uid])});
    userCollection.doc(followingUsersAccount).update({'followersCount': FieldValue.increment(-1)});
  ...
  void initState() {
    super.initState();
    enableCrashlytics();
    setCurrentScreen(widget.analytics, widget.observer, 'ProfileOthers', 'ProfileOthersState');
  }
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  InkWell(
                    onTap: () { Navigator.pushNamed(context, '/followers', arguments: {"followers": userFollowers, "username": user.username});  },
                    child: Container(
                      width: 100,
                      height: 50,
                      child: Column(
                  ...
                  InkWell(
                    onTap: () { Navigator.pushNamed(context, '/following', arguments: {"following": userFollowing, "username": user.username});  },
                    child: Container(
                      width: 100,
                      height: 50,
                      child: Column(
